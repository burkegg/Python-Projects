...
http://www.codeskulptor.org/#user44_gDh1dixJMK_3.py
...
EXPLOSION_LIFESPAN = explosion_info.get_lifespan()
...
def process_sprite_group(group, canvas):
    ...
    for i in group:
        ...
        if i.update() == True:
            removal.add(i)        
    group.difference_update(removal)
...
def group_collide(group, other_object):
    ...
    for sprite in group:
        if sprite.collide(other_object):
            ...
            my_explosion = Sprite(other_object.get_position(), [0,0], 0, 0, explosion_image, explosion_info, explosion_sound)    
...
def group_group_collide(missiles, rocks):
    ...
    for missile in missiles:
        if group_collide(rocks, missile):
            ...
            collide = True
    missiles.difference_update(remove_missiles)
    return collide
...
def draw(canvas):
    global time, started, lives, score, explosion_group
    soundtrack.play()
    # animiate background
    if started:
        time += 1
    ...
    if started and lives < 1:
        started = False
        s = rock_group
        rock_group.difference_update(s)
        timer.stop()
        explosion_group = set([])
...
def rock_spawner():
    ...
    ship_loc = my_ship.get_position()
    if dist(ship_loc, rock_pos) > 150:
